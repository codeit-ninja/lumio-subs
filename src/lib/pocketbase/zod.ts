...
export const plansSchema = z.object({
    ...
    limitPerSecond: z.number().int().min(1).refine((n) => n !== 0),
    limitPerMinute: z.number().int().min(1).refine((n) => n !== 0),
    limitPerHour: z.number().int().min(1).refine((n) => n !== 0),
    limitPerDay: z.number().int().min(1).refine((n) => n !== 0),
    limitPerMonth: z.number().int().min(1).refine((n) => n !== 0),
})
...
export const subscriptionsSchema = z.object({
    ...
    plan: z.string().regex(/^[a-z0-9]+$/).length(15),
    stripeCustomerId: z.string().max(255).optional(),
...
export const mediaSchema = z.object({
    collectionId: z.literal('pbc_2708086759').optional(),
    collectionName: z.string().min(1).max(255).optional(),
    id: z.string().regex(/^[a-z0-9]+$/).length(15).optional(),
    imdbId: z.string().min(1).max(32),
    tmdbId: z.number().int().min(1).optional(),
...
export const subtitleSearchesSchema = z.object({
    ...
    collectionName: z.string().min(1).max(255).optional(),
    id: z.string().regex(/^[a-z0-9]+$/).length(15).optional(),
    media: z.string().regex(/^[a-z0-9]+$/).length(15).optional(),
    imdbId: z.string().max(32).optional(),
    tmdbId: z.number().int().min(1).optional(),
    season: z.number().int().min(0).optional(),
    episode: z.number().int().min(0).optional(),
